...
#include "MessagesStatusWidget.h"
...
//----------------------------------------------------------------------------
// MainWidget -- UI wrapper form.

// Kind of singleton reference.
MainWidget *MainWidget::g_pMainForm = NULL;
	...
	: QWidget(pParent, wflags)
{
    ...
    m_pPatchbayRack = new PatchbayRack();
...
void MainWidget::closeEvent ( QCloseEvent *pCloseEvent )
{
...
void MainWidget::customEvent ( QEvent *pEvent )
{
...
QString MainWidget::formatExitStatus ( int iExitStatus ) const
{
	QString sTemp = " ";
...
void MainWidget::shellExecute ( const QString& sShellCommand, const QString& sStartMessage, const QString& sStopMessage )
{
...
// Stop jack audio server...
void MainWidget::stopJackServer ()
{
...
void MainWidget::appendStdoutBuffer ( const QString& s )
{
...
void MainWidget::flushStdoutBuffer ()
{
...
void MainWidget::jackError ( QProcess::ProcessError error )
{
	qjackctl_on_error(error);
}

void MainWidget::jackFinished ()
{
...
QString& MainWidget::detectXrun ( QString& s )
{
	if (m_iXrunSkips > 1)
		return s;
...
void MainWidget::updateXrunStats ( float fXrunLast )
{
	if (m_iXrunStats > 0) {
...
void MainWidget::stdoutNotifySlot ( int fd )
{
	char achBuffer[1024];
...
void MainWidget::appendMessages ( const QString& s )
{
	if (m_pMessagesStatusForm)
		m_pMessagesStatusForm->appendMessages(s);
}

void MainWidget::appendMessagesColor ( const QString& s, const QString& c )
{
	if (m_pMessagesStatusForm)
		m_pMessagesStatusForm->appendMessagesColor(s, c);
}

void MainWidget::appendMessagesText ( const QString& s )
{
	if (m_pMessagesStatusForm)
		m_pMessagesStatusForm->appendMessagesText(s);
}

void MainWidget::appendMessagesError ( const QString& s )
{
	if (m_pMessagesStatusForm) {
		m_pMessagesStatusForm->setTabPage(
            int(MessagesStatusWidget::MessagesTab));
...
void MainWidget::updateMessagesFont ()
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::updateMessagesLimit ()
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::updateJackClientPortAlias ()
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::updateDisplayEffect ()
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::updateTimeDisplayFonts ()
{
	QFont font;
...
void MainWidget::updateTimeDisplayToolTips ()
{
...
void MainWidget::updateTimeFormat ()
{
...
bool MainWidget::isActivePatchbay ( const QString& sPatchbayPath ) const
{
	bool bActive = false;
...
// Force update of active patchbay definition profile, if applicable.
void MainWidget::updateActivePatchbay ()
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::setActivePatchbay ( const QString& sPatchbayPath )
{
	if (m_pSetup == NULL)
		return;
...
void MainWidget::setRecentPatchbays ( const QStringList& patchbays )
{
...
void MainWidget::stabilize ( int msecs )
{
	QTime t;
	t.start();
...
void MainWidget::resetXrunStats ()
{
...
void MainWidget::updateXrunCount ()
{
...
QString MainWidget::formatTime ( float secs ) const
{
...
QString MainWidget::formatElapsedTime ( int iStatusItem,
...
// Update the XRUN last/elapsed time item.
void MainWidget::updateElapsedTimes ()
{
...
void MainWidget::refreshXrunStats ()
{
	updateXrunCount();
...
void MainWidget::portNotifyEvent ()
{
...
void MainWidget::xrunNotifyEvent ()
{
...
void MainWidget::buffNotifyEvent ()
{
...
void MainWidget::shutNotifyEvent ()
{
	// Log this event.
...
// Process error slot
void MainWidget::exitNotifyEvent ()
{
...
void MainWidget::jackConnectChanged ()
{
...
void MainWidget::alsaConnectChanged ()
{
...
void MainWidget::cableConnectSlot (
...
void MainWidget::startJackClientDelay ()
{
...
bool MainWidget::startJackClient ( bool bDetach )
{
...
void MainWidget::stopJackClient ()
{
...
jack_client_t *MainWidget::jackClient () const
{
...
void MainWidget::refreshConnections ()
{
	refreshJackConnections();
	refreshAlsaConnections();
}

void MainWidget::refreshJackConnections ()
{
#if 0
...
void MainWidget::refreshAlsaConnections ()
{
#if 0
	// Hack this as for a while...
...
void MainWidget::refreshPatchbay ()
{
	// Just increment our intentions; it will be deferred
	// to be executed just on timer slot processing...
	m_iPatchbayRefresh++;
...
void MainWidget::toggleMainForm ()
{
	m_pSetup->saveWidgetGeometry(this, true);
...
void MainWidget::toggleMessagesStatusForm ()
{
	if (m_pMessagesStatusForm) {
		m_pSetup->saveWidgetGeometry(m_pMessagesStatusForm);
...
void MainWidget::toggleMessagesForm ()
{
	if (m_pMessagesStatusForm) {
...
void MainWidget::toggleStatusForm ()
{
	if (m_pMessagesStatusForm) {
		int iTabPage = m_pMessagesStatusForm->tabPage();
		m_pMessagesStatusForm->setTabPage(
            int(MessagesStatusWidget::StatusTab));
		if (m_pMessagesStatusForm->isVisible()
			&& iTabPage != m_pMessagesStatusForm->tabPage())
			return;
	}
...
void MainWidget::toggleSessionForm ()
{
	if (m_pSessionForm) {
		m_pSetup->saveWidgetGeometry(m_pSessionForm);
...
void MainWidget::toggleConnectionsForm ()
{
	if (m_pConnectionsForm) {
		m_pSetup->saveWidgetGeometry(m_pConnectionsForm);
...
void MainWidget::togglePatchbayForm ()
{
	if (m_pPatchbayForm) {
		m_pSetup->saveWidgetGeometry(m_pPatchbayForm);
...
// Transport rewind.
void MainWidget::transportRewind ()
{
...
// Transport backward.
void MainWidget::transportBackward ()
{
#ifdef CONFIG_JACK_TRANSPORT
	if (m_pJackClient) {
...
void MainWidget::transportPlay ( bool bOn )
{
...
void MainWidget::transportStart ()
{
#ifdef CONFIG_JACK_TRANSPORT
	if (m_pJackClient) {
...
// Transport forward.
void MainWidget::transportForward ()
{
#ifdef CONFIG_JACK_TRANSPORT
	if (m_pJackClient) {
		jack_position_t tpos;
...
// Status item updater.
void MainWidget::updateStatusItem( int iStatusItem, const QString& sText )
{
	switch (iStatusItem) {
...
void MainWidget::updateTitleStatus ()
{
	QString sTitle;
...
void MainWidget::updateServerState ( int iServerState )
{
...
void MainWidget::updateSystemTray ()
{
#ifdef CONFIG_SYSTEM_TRAY
	if (!m_pSetup->bSystemTray && m_pSystemTray) {
...
void MainWidget::systemTrayContextMenu ( const QPoint& pos )
{
	QMenu menu(this);
	QAction *pAction;
...
//  pAction->setEnabled(m_pJackClient != NULL);
	...
	pAction->setChecked(m_pMessagesStatusForm
        ...
        && m_pMessagesStatusForm->tabPage() == MessagesStatusWidget::MessagesTab);
	pAction = menu.addAction(QIcon(":/images/status1.png"),
		tr("St&atus"), this, SLOT(toggleStatusForm()));
	pAction->setCheckable(true);
	pAction->setChecked(m_pMessagesStatusForm
		&& m_pMessagesStatusForm->isVisible()
        && m_pMessagesStatusForm->tabPage() == MessagesStatusWidget::StatusTab);
...
void MainWidget::showDirtySettingsWarning ()
{
...
void MainWidget::showDirtySetupWarning ()
{
	const QString& sTitle
...
void MainWidget::activatePresetsMenu ( QAction *pAction )
{
...
void MainWidget::activatePreset ( const QString& sPreset )
{
...
void MainWidget::contextMenuEvent ( QContextMenuEvent *pEvent )
{
...
void MainWidget::mousePressEvent(QMouseEvent *pMouseEvent)
{
...
bool MainWidget::resetDBusEngineParameter ( const QString& param )
{
	return resetDBusParameter(QStringList() << "engine" << param);
}

bool MainWidget::resetDBusDriverParameter ( const QString& param )
{
	return resetDBusParameter(QStringList() << "driver" << param);
}

bool MainWidget::resetDBusParameter ( const QStringList& path )
{
	if (m_pDBusConfig == NULL)
		return false;
...
QVariant MainWidget::getDBusEngineParameter ( const QString& param )
{
	return getDBusParameter(QStringList() << "engine" << param);
}

QVariant MainWidget::getDBusDriverParameter ( const QString& param )
{
	return getDBusParameter(QStringList() << "driver" << param);
}

QVariant MainWidget::getDBusParameter ( const QStringList& path )
{
	if (m_pDBusConfig == NULL)
		return QVariant();
...
QString MainWidget::formatQuoted ( const QString& s ) const
{
	const QChar b = ' ';
	const QChar q = '"';
...
void MainWidget::transportPlayStatus ( bool bOn )
{
	++m_iTransportPlay;
...
void MainWidget::setQuitForce ( bool bQuitForce )
{
	m_bQuitForce = bQuitForce;
}

bool MainWidget::isQuitForce () const
{
	return m_bQuitForce;
}
