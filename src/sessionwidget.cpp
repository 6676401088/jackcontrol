...
QString qjackctlSessionInfraClientItemEditor::text () const
{
...
void qjackctlSessionInfraClientItemEditor::browseSlot ()
{
...
void qjackctlSessionInfraClientItemEditor::resetSlot ()
{
...
void qjackctlSessionInfraClientItemEditor::finishSlot ()
{
	bool bBlockSignals = m_pItemEdit->blockSignals(true);
	emit finishSignal();
	m_index = QModelIndex();
...
void qjackctlSessionInfraClientItemDelegate::commitEditor ()
{
	qjackctlSessionInfraClientItemEditor *pItemEditor
...
bool SessionWidget::queryClose ()
{
	...
	if (m_pSetup && bQueryClose) {
        ...
        Session::InfraClientList& list = m_pSession->infra_clients();
		...
		for (int i = 0; i < iItemCount; ++i) {
			...
			if (pItem) {
				...
				if (!sValue.isEmpty()) {
                    Session::InfraClientItem *pInfraClientItem
                        = new Session::InfraClientItem;
...
const QStringList& SessionWidget::sessionDirs () const
{
...
bool SessionWidget::isSaveSessionVersion () const
{
...
QMenu *SessionWidget::recentMenu () const
{
...
// Save menu accessor.
QMenu *SessionWidget::saveMenu () const
{
	return m_pSaveMenu;
}
...
void SessionWidget::showEvent ( QShowEvent *pShowEvent )
{
...
void SessionWidget::hideEvent ( QHideEvent *pHideEvent )
{
...
void SessionWidget::closeEvent ( QCloseEvent * /*pCloseEvent*/ )
{
	QWidget::hide();
...
void SessionWidget::recentSession ()
{
...
void SessionWidget::saveSessionSave ()
{
	saveSessionEx(0);
}

void SessionWidget::saveSessionSaveAndQuit ()
{
	saveSessionEx(1);
}

void SessionWidget::saveSessionSaveTemplate ()
{
	saveSessionEx(2);
}

void SessionWidget::saveSessionEx ( int iSessionType )
{
...
// Save current session to specific file path.
void SessionWidget::saveSessionVersion ( bool bOn )
{
...
void SessionWidget::updateRecent ( const QString& sSessionDir )
{
...
void SessionWidget::updateRecentMenu ()
{
...
void SessionWidget::clearRecentMenu ()
{
	m_sessionDirs.clear();
	updateRecentMenu();
}
...
// Open/load session from specific file path.
void SessionWidget::loadSessionDir ( const QString& sSessionDir )
{
...
// Save current session to specific file path.
void SessionWidget::saveSessionDir (
...
QIcon SessionWidget::iconStatus ( const QIcon& icon, bool bStatus )
{
...
void SessionWidget::updateSessionView ()
{
    ...
    Session::ClientList::ConstIterator iterClient
		= m_pSession->clients().constBegin();
	for ( ; iterClient != m_pSession->clients().constEnd(); ++iterClient) {
        Session::ClientItem *pClientItem = iterClient.value();
...
void SessionWidget::sessionViewContextMenu ( const QPoint& pos )
{
	QMenu menu(this);
	QAction *pAction;
...
void SessionWidget::addInfraClient ()
{
...
void SessionWidget::editInfraClient ()
{
#ifdef CONFIG_DEBUG_0
...
void SessionWidget::editInfraClientCommit ()
{
#ifdef CONFIG_DEBUG_0
	...
	if (pItem) {
		...
		if (!sKey.isEmpty()) {
			const QString& sValue = pItem->text(1);
            Session::InfraClientItem *pInfraClientItem = NULL;
            Session::InfraClientList& list = m_pSession->infra_clients();
            Session::InfraClientList::Iterator iter	= list.find(sKey);
			if (iter == list.end()) {
                pInfraClientItem = new Session::InfraClientItem;
				pInfraClientItem->client_name = sKey;
				pInfraClientItem->client_command = sValue;
				list.insert(sKey, pInfraClientItem);
			} else {
...
void SessionWidget::removeInfraClient ()
{
#ifdef CONFIG_DEBUG_0
...
void SessionWidget::selectInfraClient ()
{
#ifdef CONFIG_DEBUG_0
...
void SessionWidget::updateInfraClients ()
{
#ifdef CONFIG_DEBUG_0
	...
	QTreeWidgetItem *pItem = NULL;
    Session::InfraClientList& list = m_pSession->infra_clients();
    Session::InfraClientList::ConstIterator iter = list.constBegin();
    const Session::InfraClientList::ConstIterator& iter_end
		= list.constEnd();
	for( ; iter != iter_end; ++iter) {
        Session::InfraClientItem *pInfraClientItem = iter.value();
		pItem = new QTreeWidgetItem(m_ui.InfraClientListView, pItem);
...
void SessionWidget::infraClientContextMenu ( const QPoint& pos )
{
	QMenu menu(this);
	QAction *pAction;
...
void SessionWidget::stabilizeForm ( bool bEnabled )
{
...
void SessionWidget::keyPressEvent ( QKeyEvent *pKeyEvent )
{
#ifdef CONFIG_DEBUG_0
